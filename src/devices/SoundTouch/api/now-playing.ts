...
import {
  ...
  connectionStatusInfoFromElement,
} from './connection-status-info.js';
import { Time, timeFromElement } from './time.js';
import { Art, artFromElement } from './art.js';
...
export function nowPlayingFromElement(
  element: XMLElement
): NowPlaying | undefined {
  if (
    !element.hasAttributes(['deviceID', 'source']) ||
  ...
  return {
    ...
    shuffleSetting: shuffleSetting
      ? (shuffleSetting as ShuffleStatus)
      : undefined,
    ...
    connectionStatusInfo: csiElement
      ? connectionStatusInfoFromElement(csiElement)
      : undefined,
  };
}
