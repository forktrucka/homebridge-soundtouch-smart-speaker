...
import { KeyState, KeyValue } from './special-types.js';
import { NowPlaying, nowPlayingFromElement } from './now-playing.js';
import { Volume, volumeFromElement } from './volume.js';
import { Sources, sourcesFromElement } from './source.js';
import { Zone, zoneFromElement, zoneToElement } from './zone.js';
import {
  BassCapabilities,
  bassCapabilitiesFromElement,
...
export class API {
  ...
  async getSource(): Promise<string | undefined> {
    ...
    if (element.hasChild('nowPlaying')) {
      const nowPlaying = element.getChild('nowPlaying');
      if (nowPlaying) {
        return nowPlaying.getAttribute('source');
      }
    }
    return undefined;
  }
  ...
  async selectSource(contentItem: ContentItem): Promise<boolean> {
    const element = await this._post(
      Endpoints.select,
      contentItemToElement(contentItem).data
    );
    return element ? element.getText('status') !== undefined : false;
  }
  ...
  private async _req(
    ...
    try {
      if (method === 'GET') {
        ...
        xml = (
          await this.axiosInstance.post(url, this.builder.buildObject(body))
        ).data;
      }
